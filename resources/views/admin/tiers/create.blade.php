    ...
    <div class="min-w-0 flex-1">
        ...
        <div class="mt-1 flex flex-col sm:mt-0 sm:flex-row sm:flex-wrap sm:space-x-6">
            <div class="mt-2 flex items-center text-sm text-gray-500">
                Add a new partner tier, set quota, price, and allowed packages
            </div>
        </div>
    </div>
...
<div class="bg-white shadow rounded-lg p-6 mb-8 w-full">
    <form method="POST" action="{{ route('admin.tiers.store') }}" class="space-y-8 w-full">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            ...
            <div>
                ...
                <input type="number" name="request_quota" id="request_quota" min="1" value="{{ old('request_quota') }}"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                       placeholder="e.g., 20">
                @error('request_quota')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="md:col-span-2">
                ...
                <select name="inspection_packages[]" id="inspection_packages" multiple
                    ...
                    @foreach($packages as $package)
                        <option value="{{ $package->id }}" {{ in_array($package->id, old('inspection_packages', [])) ? 'selected' : '' }}>
