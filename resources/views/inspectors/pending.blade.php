                ...
                @foreach($pendingRequests as $request)
                    <li class="p-4">
                        <div class="font-semibold text-indigo-700">
                            ...
                            <span class="ml-2 px-2 py-1 rounded text-xs font-semibold bg-indigo-100 text-indigo-800">Assigned</span>
                        </div>
                        <div class="text-xs text-gray-500 mb-2">{{ $request->property->address ?? '-' }} | {{ ucfirst($request->urgency) }} | {{ $request->package->display_name ?? '-' }}</div>
                        <div class="text-xs text-gray-400">Scheduled: {{ $request->scheduled_date }} {{ $request->scheduled_time }}</div>
                    </li>
                @endforeach
            </ul>
        @else
            <div class="text-gray-400 text-center py-8">No pending requests found.</div>
        @endif
    </div>
</div>
@endsection 