...
Route::domain('{partner}.electronova.rw')->middleware(['detect_partner_subdomain'])->group(function () {
    // Login routes
    ...
    Route::get('/', function () {
        if (auth()->check()) {
            return redirect()->route('partner.dashboard', ['partner' => request()->route('partner')]);
        }
        return redirect()->route('partner.login', ['partner' => request()->route('partner')]);
    })->name('partner.home');
    ...
    Route::middleware(['auth'])->group(function () {
        ...
        Route::get('billing', [BillingController::class, 'index'])->name('partner.billing');
        Route::post('billing/select-tier/{tier}', [BillingController::class, 'selectTier'])->name('partner.billing.select-tier');
    ...
    Route::get('billing/flutterwave/callback', [BillingController::class, 'flutterwaveCallback'])->name('partner.billing.flutterwave.callback');

    // Test route for partner subdomain
